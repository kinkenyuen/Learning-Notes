...
由于顶层autorelease池在线程退出之前不会释放其对象，因此**常驻线程应该创建额外的autorelease池来更频繁地释放对象。**例如，**使用runloop的线程可以每次通过runloop创建并释放一个自动释放池。更频繁地释放对象可以防止应用程序的内存占用过大，从而导致性能问题。**但是，对于任何与性能相关的行为，您都应该度量代码的实际性能，并适当地调优您对自动释放池的使用。

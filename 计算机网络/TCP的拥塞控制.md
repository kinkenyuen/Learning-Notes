...
为了防止拥塞窗口cwnd增长过引起网络拥塞，还需要设置一个**慢开始门限ssthresh**状态变量，它的用法如下：
...
当拥塞窗口cwnd = 16时(图中的点4)，出现了一个新的情况，就是发送方一连收到3个对统一报文段的重复确认(图中记为3-ACK)。关于这个问题解释：**有时，个别报文段会在网络中丢失，但实际上网络并未发生拥塞。如果发送方迟迟收不到确认，就会产生超时，就会误认为网络发生了拥塞，导致发送方错误地启动慢开始，把拥塞窗口cwnd又设置为1，因而降低了传输效率**

## 快重传和快恢复

采用快重传算法可以让发送方尽早知道发生了个别报文段
...
**TCP采用一种自适应算法，它记录一个报文段发出的时间，以及收到相应的确认的时间**。这两个时间只差就是**报文段的往返时间RTT**。TCP保留了RTT的一个**加权平均往返时间RTTs**(又称为平滑的往返时间，S表示Smoothed)。简单来说就是一个平均值。RTTs根据每次新的RTT来更新。显然，超时计时器设置的超时重传时间RTO应略大于RTTs
